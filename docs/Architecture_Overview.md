...
This document gives a detailed subsystem-level view of how Chimera is structured in v0.5.1 and how data moves through the system.
...
6. enrich scoring with identity sequence, relationship, and campaign signals
7. emit reports, examples, and benchmark artifacts
...
### 2.4 `chimera.detectors` and `chimera.model`

Responsibilities:

- provide detector implementations
- manage model configuration and persistence
- expose a stable scoring interface to the CLI and benchmark harness
...
### 2.8 `chimera.reporting`

Responsibilities:

- serialize result artifacts
- generate benchmark JSON
- produce paper-style Markdown reports

### 2.9 Native acceleration path
...
The system does not attempt a generic XAI layer. It exposes the actual deterministic reasons that were used to create the score contribution.
...
- additive rather than destructive subsystem evolution
- optional native acceleration without mandatory dependency burden

## 8. Current Architectural Pressure Points
...
- additional documentation around output artifacts and operational expectations
